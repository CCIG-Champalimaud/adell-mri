"""
Test the new GP-aware predict_step implementation.
"""
...
def test_gp_predict_step():
    """Test the new GP-aware predict_step"""
    print("Testing GP-aware predict_step...")
    ...
    # Test 1: Standard prediction (no uncertainty)
    print("  Test 1: Standard prediction...")
    prediction = pl_module.predict_step(batch, 0)
    assert prediction.shape == (batch_size, output_dim)
    print("    ✓ Standard prediction works")
    ...
    # Test 3: Prediction with uncertainty using dedicated GP method
    print("  Test 3: Prediction with uncertainty using dedicated GP method...")
    ...
    # Test 4: Non-GP model (should return standard prediction)
    print("  Test 4: Non-GP model...")
    network_no_gp = SimpleGPNetwork(
        input_dim, hidden_dim, output_dim, use_gp=False
    )
    pl_module_no_gp = TestGPPLModule(network_no_gp)

    result_no_gp = pl_module_no_gp.predict_step(
        batch, 0, return_uncertainty=True
    )
    assert torch.is_tensor(result_no_gp), "Non-GP model should return tensor"
    assert result_no_gp.shape == (batch_size, output_dim)
    print("    ✓ Non-GP model fallback works")

    # Test 5: GP not fitted (should return standard prediction)
    print("  Test 5: GP not fitted...")
    ...
    result_unfitted = pl_module_unfitted.predict_step(
        batch, 0, return_uncertainty=True
    )
    assert torch.is_tensor(result_unfitted), "Unfitted GP should return tensor"
    print("    ✓ Unfitted GP fallback works")

    print("\n🎉 All GP predict_step tests passed!")


def test_multiclass_gp_predict_step():
    """Test GP predict_step with multiclass"""
    print("Testing multiclass GP predict_step...")
    ...
    result = pl_module.predict_step(
        batch, 0, return_uncertainty=True, n_samples=n_samples
    )
    ...
    print("  ✓ Multiclass GP prediction works")
    print("🎉 Multiclass test passed!")


if __name__ == "__main__":
    test_gp_predict_step()
    test_multiclass_gp_predict_step()
    print("\n✅ All GP predict_step tests completed successfully!")
