...
def test_normalised_activations(act_str):
    ...
    assert list(output.shape) == list(
        input_tensor.shape
    ), "output shape does not match input shape"
    assert torch.all(torch.isnan(output)) == False, "operation produced nan"
    assert torch.all(torch.isinf(output)) == False, "operation produced inf"
