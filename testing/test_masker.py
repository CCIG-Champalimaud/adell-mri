...
def test_masker_transformer(n_dim,with_mask):
    ...
    masker = get_masker("transformer",
                        image_dimensions=image_dimensions,
                        min_patch_size=min_patch_size,
                        max_patch_size=max_patch_size)
    n_tokens = int(np.prod(image_dimensions))
    if with_mask == True:
        masker(torch.ones(1,n_tokens,n_channels),mask_vector)
    else:
        masker(torch.ones(1,n_tokens,n_channels))
...
def test_masker_convolutional(n_dim,with_mask):
    if n_dim == 2:
        ...
        min_patch_size = [1,1,1]
        max_patch_size = [4,4,4]
    masker = get_masker("convolutional",
                        image_dimensions=image_dimensions,
                        min_patch_size=min_patch_size,
                        max_patch_size=max_patch_size)
    if with_mask == True:
        masker(torch.ones(1,n_channels,*image_dimensions),mask_vector)
    else:
        masker(torch.ones(1,n_channels,*image_dimensions))
