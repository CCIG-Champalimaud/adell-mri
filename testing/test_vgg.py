...
import torch
from lib.modules.classification import VGG
...
def test_vgg_names():
    ...
    set_diff = set.difference(set(vgg_mod_be_param_names),
                              set(vgg_mod_param_names))
    assert len(set_diff) == len(vgg_mod_be_param_names) - len(vgg_mod_param_names)