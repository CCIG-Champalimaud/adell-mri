...
from lib.modules.classification.classification import GenericEnsemble
from lib.modules.layers.adn_fn import get_adn_fn,ActDropNormBuilder
...
@pytest.mark.parametrize("sd,sae",
                         [(2,False),
                          (2,True),
                          (3,False),
                          (3,True)])
def test_ensemble_unet_encoders(sd,sae):
    ...
    ge = GenericEnsemble(
        spatial_dimensions=sd,networks=networks,n_classes=n_classes,
        n_features=[depth[-1] for _ in range(n_elements)],
        head_structure=[64,64],
        head_adn_fn=head_adn_fn,sae=sae)
    
    input_tensor = torch.rand(input_shape[:(2+sd)])
    input_tensor_list = [torch.rand(input_shape[:(2+sd)])
                         for _ in range(n_elements)]
    out = ge(input_tensor)
