...
if __name__ == "__main__":
    ...
    ah = AugmentationWorkhorsed(
        all_augments, ["image"], N=2, dropout_size=(32, 32)
    )
    ...
    for k in ah.transforms:
        ...
        with tqdm(range(N)) as pbar:
            for i in pbar:
                pbar.set_description("transform={}; iteration={}".format(k, i))
    ...
    print(
        "Average time per transform:", time_elapsed / (len(ah.transforms) * N)
    )
