...
from adell_mri.modules.layers.utils import split_int_into_n
        ...
        for pyr_op, up_op, x in zip(
            ...
            if self.shape_check is False:
                if prev_x.shape != x.shape:
                    prev_x = F.interpolate(prev_x, x.shape[2:], mode="nearest")
            prev_x = x + prev_x
        return prev_x
...
class ReceptiveFieldBlock2d(torch.nn.Module):
    def __init__(
        ...
        self.out_c_list = split_int_into_n(self.out_channels, len(self.rates))
