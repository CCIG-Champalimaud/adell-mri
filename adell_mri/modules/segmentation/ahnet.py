...
class AHNet(torch.nn.Module):
    ...
    def init_layers_2d(self):
        ...
        if self.n_classes == 2:
            self.final_layer = torch.nn.Sequential(
                torch.nn.Conv2d(n_out, 1, 1), torch.nn.Sigmoid()
            )
        else:
            self.final_layer = torch.nn.Sequential(
                torch.nn.Conv2d(n_out, self.n_classes, 1), torch.nn.Softmax(1)
            )
    ...
    def init_layers_3d(self):
        ...
        self.max_pool_1_3d = torch.nn.Sequential(
            torch.nn.MaxPool3d([1, 1, 2], stride=[1, 1, 2], padding=[0, 0, 1]),
            torch.nn.MaxPool3d([3, 3, 3], stride=[2, 2, 2], padding=[1, 1, 0]),
        )
        ...
        if self.n_classes == 2:
            ...
            self.final_layer_3d = torch.nn.Sequential(
                torch.nn.Conv3d(
                    n_out * len(self.psp_levels) + n_out, self.n_classes, 1
                ),
                torch.nn.Softmax(1),
            )
