...
from lib.utils import (
    ...
    RandomSlices,
    SlicesToFirst)
from lib.modules.segmentation_pl import AHNetPL
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--checkpoint_restore','checkpoint_restore',type=str,default=None,
        help="Path to checkpoint that will be restored prior to training.")
    parser.add_argument(
        '--spatial_dim_training','spatial_dim_training',type=int,default=2,
        help="Spatial dimension during training.")
    ...
    for val_fold in range(args.n_folds):
        ...
        ahnet = AHNetPL(training_dataloader_call=train_loader_call,
        ...
        if args.checkpoint_restore is not None:
            state_dict = torch.load(
                args.checkpoint_restore,map_location=args.dev)['state_dict']
            ahnet.load_state_dict(state_dict)
        if args.spatial_dim_training != ahnet.spatial_dim:
            if ahnet.spatial_dim == 3:
                raise Exception("Can only convert from 2D to 3D")
            else:
                ahnet.convert_to_3d()
