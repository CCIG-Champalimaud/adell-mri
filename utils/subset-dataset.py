...
def remove_objects(mask):
    ...
    while nb_objects > 1:
        ...
        new_arr = remove_small_holes(
            ...
            arr, min_size=min_size, connectivity=3), area_threshold=min_size, connectivity=3)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--mask_idx",dest="mask_idx",type=int,nargs="+",default=[])
    ...
    file_dict = {
        k:file_dict[k] for k in file_dict
    ...
    if args.subsample_size is not None:
        ...
        file_dict = {k:file_dict[k] for k in rng.choice(file_dict_keys,size=args.subsample_size)}
    ...
    for k in tqdm(file_dict):
        for file_key in file_dict[k]:
            ...
            output_path = os.path.join(args.output_path,k,file_name + ".nii.gz")
