...
if __name__ == "__main__":
    ...
    parser.add_argument("--path",
                        required=True)
    parser.add_argument("--structure",nargs=2,type=int,
                        required=True)
    ...
    for output_path in args.output_path:
        ...
        path_subset = np.random.choice(
            all_dcm_paths,size=n,replace=False)
        ...
        for i,path in enumerate(path_subset):
            ...
            all_dcm[i // w].append(np.uint8(resize(dcm,args.size,anti_aliasing=True) * 255))

        all_dcm = [np.concatenate(dcm_list,axis=0)
                for dcm_list in all_dcm]
