...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--input_paths",dest="input_paths",action="store",
        nargs='+',help="Paths to JSON files.",required=True)
    parser.add_argument(
        "--suffixes",dest="suffixes",action="store",nargs='+',
        help="When solving conflicts uses these suffixes.",default=None)
    parser.add_argument(
        "--rename",dest="rename",action="store",
        help="Renames old fields to new names (format: old_name,new_name).",
        default=None,nargs='+')
    parser.add_argument(
        "--keep_all",dest="keep_all",action="store_true",
    ...
    dict_list = [
        json.load(open(x)) for x in args.input_paths]
    ...
    if args.rename is None:
        ...
        args.rename = dict(
            [tuple(k.split(',')) for k in args.rename])
