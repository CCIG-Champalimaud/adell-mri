...
if __name__ == "__main__":
    ...
    parser.add_argument(
        '--input_shape',dest='input_shape',type=int,nargs="+",
        help="Input shape",required=True)
    parser.add_argument(
        '--n_channels',dest='n_channels',type=int,
        help="Number of input channels",required=True)
    parser.add_argument(
        '--n_classes',dest='n_classes',type=int,
        help="Number of classes",required=True)
    parser.add_argument(
        '--n_clinical_features',dest='n_clinical_features',type=int,
        help="Number of clinical features",default=0)
    parser.add_argument(
        '--config_file',dest="config_file",
        help="Path to network configuration file (yaml)",
        required=True)
    parser.add_argument(
        '--checkpoint',dest="checkpoint",help="Path to checkpoint",
        required=True)
    parser.add_argument(
        '--output_path',dest="output_path",help="Path to traced model",
        required=True)
    parser.add_argument(
    ...
    if args.net_type == "unet":
        network_config,_ = parse_config_unet(args.config_file,
                                             args.n_channels,args.n_classes)
    else:
    ...
    network = get_classification_network(
        ...
        keys=[
            "image_{}".format(i) for i in range(args.n_channels)],
    ...
    state_dict = torch.load(
        args.checkpoint,
        map_location=args.dev)['state_dict']
