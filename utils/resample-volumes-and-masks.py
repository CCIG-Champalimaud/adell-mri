...
def resample_to_target(image,fixed_image,is_label=False):
    ...
    return sitk.Resample(image, fixed_image, sitk.Transform(),
                         interpolator, 0.0)
...
if __name__ == "__main__":
    ...
    for pattern in args.image_patterns:
        ...
        image_paths = [x for x in image_paths
                       if compiled_pattern.search(x) is not None]
    ...
    for pattern in args.mask_patterns:
        ...
        mask_paths = [x for x in mask_paths
                      if compiled_pattern.search(x) is not None]
    ...
    for identifier in tqdm(path_dictionary):
        ...
        fixed_image = resample_image(
            fixed_image,args.spacing,args.is_label)
        sitk.WriteImage(fixed_image,os.path.join(
            output_base,path_curr.split(os.sep)[-1]))
        ...
        if "masks" in path_dictionary:
            for mask_path in path_dictionary[identifier]["masks"]:
                ...
                sitk.WriteImage(image,
                                os.path.join(
                                    output_base,mask_path.split(os.sep)[-1]),
