...
from lib.modules.object_detection import CoarseDetector3d
from lib.modules.object_detection_pl import CoarseDetector3dPL
from lib.modules.layers import ActDropNorm
...
if __name__ == "__main__":
    ...
    if "classification_loss_fn" in network_config:
        del network_config["classification_loss_fn"]
    ...
    output_example = CoarseDetector3d(
        n_channels=1,activation_fn=torch.nn.Identity,
        anchor_sizes=anchor_array,maxpool_structure=maxpool_structure,
        dev=args.dev)(torch.ones([1,1,*input_size]))
    output_sh = output_example.shape[2:]
    ...
    if args.augment == True:
        augments = [
            ...
            RandomAffined(
                keys=[*keys,"bb_map"],
                spatial_sizes=[input_size,output_sh],
                mode=["bilinear","nearest"],prob=0.1,
                rotate_range=[np.pi/6,np.pi/6,np.pi/6],
                shear_range=[0,0,0],translate_range=[10,10,3],
    ...
    for val_fold in range(args.n_folds):
        ...
        object_loss_params = get_loss_param_dict(
            object_weights,args.loss_gamma,
            args.loss_comb,0.,
        ...
        trainer.fit(detector, train_loader, train_val_loader)

        # assessing performance on validation set
        print("Validating...")
        
        test_metrics = trainer.test(detector,validation_loader)[0]
