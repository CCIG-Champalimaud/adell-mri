...
class ReceptiveFieldBlock(torch.nn.Module):
    def __init__(self,in_channels:int,out_channels:int,
                 rates:List[int],
                 act_fn:torch.nn.Module=torch.nn.ReLU)->torch.nn.Module:
...
class ChannelSqueezeAndExcite3d(torch.nn.Module):
    def __init__(self,input_channels:int)->torch.nn.Module:
        super(self).__init__()
        self.input_channels = input_channels
...
class SCSAE3d(torch.nn.Module):
    ...
    def init_layers(self):
        ...
        self.channel = ChannelSqueezeAndExcite3d(self.input_channels)
...
class YOLO3d9000(torch.nn.Module):
