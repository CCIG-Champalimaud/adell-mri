...
class UNetContrastiveSemiSL(UNetSemiSL,UNetBasePL):
    ...
    def training_step(self,batch,batch_idx):
        ...
        # self-supervised bit
        if self.semi_sl_image_key_1 is not None and \
            self.semi_sl_image_key_2 is not None:
            x_1,x_2,x_cond,x_fc = self.unpack_batch_semi_sl(batch)
            self_sl_loss = self.step_semi_sl(x_1,x_2,x_cond,x_fc)
            self.log("train_self_sl_loss",self_sl_loss,
    ...
    def validation_step(self,batch,batch_idx):
        ...
        if self.semi_sl_image_key_1 is not None and \
            ...
            x_1,x_2,x_cond,x_fc = self.unpack_batch_semi_sl(batch)
            self_sl_loss = self.step_semi_sl(x_1,x_2,x_cond,x_fc)
            self.log("val_self_sl_loss",self_sl_loss,
                     batch_size=y.shape[0],sync_dist=True)
            output_loss = output_loss + self_sl_loss
