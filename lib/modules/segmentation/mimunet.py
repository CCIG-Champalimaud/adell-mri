...
class MIMUNet(torch.nn.Module):
    ...
    def __init__(self,
        ...
        if any([self.depth is None,
                self.padding is None]):
            inf_par = self.infer_depth_padding_kernel_size(
                self.module)
        ...
        self.slice_to_vol = torch.nn.ModuleList([
            einops.layers.torch.Rearrange(
                ...
                s=self.n_slices,c=self.n_channels,D=d) 
            for d in self.depth])
    ...
    def infer_depth_padding_kernel_size(module: torch.nn.Module,
        ...
        kernel_size = [x + y for x,y in zip(minimum_kernel_size,
                                            diff_from_same)]
        padding = [0 if (pad > 1) else (ks // 2)
    ...
    def init_link_ops(self):
        ...
        if self.link_type == "identity":
            ...
            self.link_ops = torch.nn.ModuleList([
                torch.nn.Sequential(
                    torch.nn.Conv3d(d,d,3,padding=p),
                    self.adn_fn(d))
