...
def calculate_operating_points(y, x, op_match=None, verbose=1):
    ...
    if not np.all(np.diff(y) >= 0) and verbose:
        print(
            "Warning: y performance metric is not monotonically increasing, this could lead to unexpected behaviour!"
        )
    if not np.all(np.diff(x) >= 0) and verbose:
        print(
            "Warning: x performance metric is not monotonically increasing, this could lead to unexpected behaviour!"
        )
...
def calculate_statistics(
    ...
    eps = 1e-10
    x_shared = np.array(
        [xi for _, x in metrics for xi in x], dtype=np.float64
    )  # collect list of all possible x-values
    x_shared = np.ravel(x_shared)  # flatten list, if necessary
    x_shared = np.append(
        x_shared, [x_start, x_end]
    )  # add x_start and x_end to ensure correct pAUC calculation
    ...
    operating_points = calculate_operating_points(
        y=y_shared_mean, x=x_shared, op_match=op_match, verbose=verbose
    )
...
def calculate_pAUC_from_graph(
    x, y, pAUC_start: float = 0.0, pAUC_end: float = 1.0, full: bool = False
):
    """
    ...
    if not isinstance(x[0], (list, np.ndarray)) or not isinstance(
        y[0], (list, np.ndarray)
    ):
        # have a single set of (x,y) coordinates

        assert not isinstance(x[0], (list, np.ndarray)) and not isinstance(
            y[0], (list, np.ndarray)
    ...
    results = calculate_statistics(
        metrics=coordinates_joined, x_start=pAUC_start, x_end=pAUC_end
    )
    if full:
        return results
