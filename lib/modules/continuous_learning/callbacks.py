...
class MultiPhaseTraining(Callback):
    ...
    def assertions(self):
        ...
        """
        assert len(self.learning_rates) == len(self.n_epochs), \
            "len(learning_rates) should be the same as len(n_epochs)"
        assert all([isinstance(x,int) or x == "adaptive" 
                    for x in self.n_epochs]), \
            "n_epochs should only have integers or 'adaptive'"
        assert all([isinstance(x,list) or x == "stop"
    ...
    def on_validation_epoch_end(self,
        ...
        if self.n_epochs[self.idx] == "adaptive":
            if self.checker(trainer.callback_metrics.get(self.monitor))==True:
                changed = True
        else:
        ...
        if changed == True:
            ...
            if isinstance(lr,list):
                for param_group,lr in zip(opt.param_groups,
                                          self.learning_rates[self.idx]):
                    param_group['lr'] = lr
